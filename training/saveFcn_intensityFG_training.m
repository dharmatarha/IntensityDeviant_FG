function saveFcn_intensityFG_training(subjID, trainingType, deviantType, expName, varargin)
%% Function to save out the parameters and results of intensityFG_training
%
% USAGE: saveFcn_intensityFG_training(subjID, deviantType, expName, varargin)
% 
...
% trainingType  - Numeric value, one of 1:4, marks training type.
...
if nargin < 4
    error('Input arguments "subjID", "trainingType", "deviantType" and "expName" are mandatory!');
end
...
if ~isnumeric(trainingType) || ~ismember(trainingType, 1:4)
    error('Input arg "trainingType" should be an integer in range 1:4!');
end
...
saveFileName = [expName, '_subj', num2str(subjID), '_trainingType', num2str(trainingType), '_', dateStr, '.mat'];


%% Save

save(saveFileName,...
    'subjID', 'trainingType', 'deviantType', 'expName',...
    'output'); 
