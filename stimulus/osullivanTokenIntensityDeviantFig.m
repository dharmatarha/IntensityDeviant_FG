% [sigOut] = osullivanTokenIntensityDeviantFig(fs, cl, clNot, lenNote, lenNoteRamp, numNotesPerToken, minSemitoneStep, clITD, clNotITD, clTraj, [baseFreq, upFreq], maxSemitoneStep, clStart, [clPhaseFLAG, clNotPhaseFLAG], plotMe, baseSNR, deviantInd, deviantSNR)
%
% [sigOut, params] = osullivanTokenIntensityDeviantFig(...
% [sigOut, params, clOut] = osullivanTokenIntensityDeviantFig(...
% [sigOut, params, clOut, clNotOut] = osullivanTokenIntensityDeviantFig(...
%
...
%   baseSNR         (OPTIONAL) This value (in dB) specifies the ratio of
%                   power between the CL and the CLnot signals for the
%                   NON-DEVIANT chords. If specified as 'nan', the function
%                   calculates an estimate based on the sum(cl)/sum(clNot)
%                   ratio. 
...
%   deviantSNR      (OPTIONAL) This value (in dB) specifies the ratio of
%                   power between the CL and CLnot signals for the DEVIANT
%                   chords. This value is *relative* to 'baseSNR'!!!!  In
%                   other words, if the deviantSNR = 0, then baseSNR ==
%                   deviantSNR and there are effectively no deviants.
%                   NOTE: although this input parameter maintains
%                   the same name as in 'IntensityDeviantBack.m', the value
...
%
% ******EXAMPLE 1: single ITD used for 'cl' and 'clNot'
% fs = 48000;
% numNotesPerToken = 20;
% lenNote = 0.120; %1/13;
% cl = 3;
...
% deviantSNR = -25;
% 
% [sigOut] = osullivanTokenIntensityDeviantFig(fs, cl, clNot,  round(lenNote*fs),  round(0.010*fs),  numNotesPerToken, minSemitoneStep, clITD, clNotITD, clTraj, [baseFreq upFreq], 2, clStart, [1 1], plotMe, baseSNR, deviantInd, deviantSNR);   
%
%
% ******EXAMPLE 2: using multiple ITD values for 'cl' and 'clNot'
% fs = 48000;
% numNotesPerToken = 20;
% lenNote = 0.12; %1/13;
% cl = [3 0 3];
% clITD = {nan(1, numNotesPerToken), 400*ones(1, numNotesPerToken), zeros(1,numNotesPerToken)};    %-400*ones(1, numNotesPerToken);    
% clNot = [0 0 7]; %[1 0 0]; %[2 1 2];
...
% plotMe = 1;
% 
% baseSNR = nan;  % If specified as 'nan', the function calculates an estimate based on the sum(cl)/sum(clNot) ratio
% deviantInd = [10:13];
% deviantSNR = -25;
% 
% [sigOut] = osullivanTokenIntensityDeviantFig(fs, cl, clNot,  round(lenNote*fs),  round(0.010*fs),  numNotesPerToken, minSemitoneStep, clITD, clNotITD, clTraj, [baseFreq upFreq], 2, clStart, [1 1], plotMe, baseSNR, deviantInd, deviantSNR);   
...
function [sigOut, varargout] = osullivanTokenIntensityDeviantFig(fs, cl, clNot, lenNote, lenNoteRamp, numNotesPerToken, minSemitoneStep, clITD, clNotITD, clTraj, varargin)
...
% Can't specify a deviant that exceed the number of possible tokens
if max(deviantInd) > numNotesPerToken  
    error('DKR: The indices of the deviant cannot exceed ''numNotesPerToken''.')
end
...
%--------------------------------------------------------------------------
% Handle the deviant SNR calculation
%--------------------------------------------------------------------------
% Estimate the 'baseSNR' if it wasn't specified by the user... assuming it
% is necessay because both CL and CLnot values are specified.
if ~(sum(cl) == 0 || sum(clNot) == 0)
    if isnan(baseSNR)
        N = 10;  % Number of iterations to be used for the estimate
        [baseSNR, ~, ~] = calcExpectedCLtoCLnotRatio(N, sum(cl), sum(clNot), notesPossible, lenNote/fs, fs);
    end
    
% WARNING to the user if non-zero SNR values are specified when there are
% no components in either 'cl' or 'clNot' 
else   
    
    if isnan(baseSNR)  % Catch when the default value for 'baseSNR' is used
        baseSNR = 0;
    end
    
    if (deviantSNR ~= 0 || baseSNR ~= 0)
        warning('DKR: Values for ''baseSNR'' and ''deviantSNR'' are irrelevant if either ''cl'' or ''clNot'' do not contain tonal components.')
        
        % Set a default SNR values that will not result in errors or modification to the signal 
        baseSNR = 0;  
        deviantSNR = 0;
    end
end
...
%--------------------------------------------------------------------------
% Generate an array for the SNR
%--------------------------------------------------------------------------
% baseSNR       =>  CL-to-CLnot power ratio in dB for the standard...
%                   positive value indicates CL greater in power
% deviantInd    =>  indicies within the duration of the entire stimulus
%                   that are deviants
% deviantSNR    =>  change in CL-to-CLnot power ratio in dB from the
%                   baseSNR... positive value indicates CL greater in
%                   power
%------------------------------------------------------------
    
SNRarray = ones(numNotesPerToken,1);  % Initialize array that will hold the SNR for each chord.

SNRarray = SNRarray * baseSNR;  % set all SNR values to the standard value

SNRarray(deviantInd) = SNRarray(deviantInd) + deviantSNR;  % SUM the deviant SNR to generate the deviant value
...
for j = 1:numNotesPerToken    
    ...
    if sum(cl) ~= 0 
        ...
        if ~iscell(clITD)  % if only one value for 'clNot' is specified
            for i = 1:cl  
    ...
    if ~(sum(cl) == 0 || sum(clNot) == 0)
        tmpNoteL_cl = tmpNoteL_cl * 10^(SNRarray(j)/20);
        tmpNoteR_cl = tmpNoteR_cl * 10^(SNRarray(j)/20);
    end
